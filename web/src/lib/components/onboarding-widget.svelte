<script lang="ts">
	...
	const steps = $derived(
		[
			missingAvatar && { label: 'Upload a photo', href: '/settings/avatar', icon: BookImageIcon },
			missingName && { label: 'Add your name', href: '/settings/username', icon: UserRoundPenIcon }
...
{#if isVisible}
	<Sidebar.Group>
		<Sidebar.GroupContent class="px-1">
			<div class="rounded-lg px-3 py-2.5" style="background-color: color-mix(in srgb, #FF3E00 12%, transparent);">
				<p class="mb-2.5 text-[11px] font-semibold uppercase tracking-wider" style="color: #FF3E00;">
				...
				<div class="flex flex-col gap-1">
					{#each steps as step}
						<a
							...
							onmouseenter={(e) => (e.currentTarget.style.backgroundColor = 'color-mix(in srgb, #FF3E00 18%, transparent)')}
